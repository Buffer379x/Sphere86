/**
 * ImagePickerModal — two-tab image browser for selecting a floppy or CD-ROM image (VM settings).
 * Both tabs are read-only here (browse + select); uploads and deletes use the sidebar Media Manager.
 */
import { Fragment, useMemo, useRef, useState } from 'react'
import { createPortal } from 'react-dom'
import { useQuery } from '@tanstack/react-query'
import { ChevronRight, Disc, FolderOpen, HardDrive, Loader2, Save, X, CloudOff } from 'lucide-react'
...
function ColumnBrowser({ tree, kind, onSelect, breadcrumbRoot = 'Root' }: {
  tree: LibraryNode[]
  kind?: 'floppy' | 'cdrom'
  onSelect?: (node: LibraryNode, path: string) => void
  /** First breadcrumb segment (e.g. "My Images" in the VM settings picker). */
  ...
  const breadcrumb = useMemo(() => {
    const items: string[] = [breadcrumbRoot]
    let current = filtered
    for (const name of columnPath) {
    ...
    return items
  }, [filtered, columnPath, breadcrumbRoot])
...
export default function ImagePickerModal({ kind, currentPath, onSelect, onClear, onClose }: Props) {
  ...
  const title = kind === 'floppy' ? 'Select Floppy Image' : 'Select CD-ROM Image'
